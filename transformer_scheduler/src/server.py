...
class Server:
    ...
    def _apply_fedqsn_logic(self, model_sd, round_idx, client_id):
        ...
        with torch.no_grad():
            for name, param in proxy_sd.items():
                ...
                if (
                    param.dim() < 2 or
                    'weight' not in name or
                    not any(token in name for token in ("attn", "mlp", "attention", "ffn", "lin"))
                ):
                    p1 = 0.0
                    p2 = 0.0
                    omega_val = 0
                ...
                if p1 > 0 and target_param.numel() > 0:
                    if name not in self.server_masks:
                ...
                if p2 > 0 and target_param.numel() > 0:
                    ...
                    client_mask = mask_col.expand_as(target_param)
                    target_param = target_param * client_mask

                # ------------------------------------------------------------------
                # C. Block-wise Quantization (omega)
                # ------------------------------------------------------------------
                if omega_val > 0:
                    block_size = 256
        ...
        with torch.no_grad():
            for name, param in proxy_sd.items():
                ...
                if w > 0:
                    ...
                    # Normalize
                    z_l = torch.where(blocks.abs().max(dim=1, keepdim=True).values == 0, 
    ...
    def reset_perturbation_direction(self):
        for client_perturbation_direction in self.perturbation_direction:
            for name in client_perturbation_direction.keys():
                client_perturbation_direction[name].zero_()
        ...
        if len(valid_perturbation_direction_ids) == 0:
            return {
                client_id: {
                    ...
                    for name, tensor in self.perturbation_direction[client_id].items()
                }
    ...
    def get_model(self, round_idx=None, client_id=None, centered_noise=None):
        ...
        if self.args.DP == 'qsn':
            return self._apply_fedqsn_logic(model_sd, round_idx, client_id)
        if self.args.DP == 'lpp':
    ...
    def update_node_info(self, weight, proxy_weight, clean_weight, node_id):
        for k in weight.keys():
            ...
            if 'weight' in k or 'bias' in k:
                scale_update_tensor = (w_k - clean_weight[k]).clone()
                perturbation_direction_update_tensor = (w_k - proxy_weight[k]).clone()
                self.scale[node_id][k] = scale_update_tensor
                self.perturbation_direction[node_id][k] = perturbation_direction_update_tensor
        ...
        if self.args.DP == 'qsn' and len(self.server_masks) > 0 and self.global_init_sd is not None:
            with torch.no_grad():
                for name, mask in self.server_masks.items():
                    ...
                    expanded_mask = mask.to(device=device, dtype=dtype).expand_as(avg_parameters[name])
                    avg_parameters[name] = (
                        avg_parameters[name] * expanded_mask +
                        self.global_init_sd[name].to(device=device, dtype=dtype) * (1 - expanded_mask)
                    )
    ...
    def set_initial_model(self):
        self.model, _ = build_model(self.args)
        self.server_masks = {}
        model_sd = get_model_sd(self.model)
