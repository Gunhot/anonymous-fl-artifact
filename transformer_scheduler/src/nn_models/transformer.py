...
class Multi_Transformer(nn.Module):
    def __init__(self, n_blocks, num_tokens, embedding_size, num_heads, hidden_size, dropout, bptt):
        super().__init__()
    ...
    def forward(self, input, mask_change, mask_random):
        ...
        middle_output1 = self.decoder1(src)
        #middle_output1 = middle_output1.permute(0, 2, 1)
        if self.n_blocks == 1:
            return [middle_output1]
        src = self.encoder_layer2(src)
        middle_output2 = self.decoder2(src)
        #middle_output2 = middle_output2.permute(0, 2, 1)
        if self.n_blocks == 2:
            return [middle_output1, middle_output2]
        src = self.encoder_layer3(src)
        middle_output3 = self.decoder3(src)
        #middle_output3 = middle_output3.permute(0, 2, 1)
        if self.n_blocks == 3:
        ...
        # return out
        return [middle_output1, middle_output2, middle_output3, out]

def multi_transformer(n_blocks=4):
    ...
    dropout = 0.2
    bptt = 64  # not sure
    return Multi_Transformer(n_blocks, num_tokens, embedding_size, num_heads, hidden_size, dropout, bptt)


if __name__ == "__main__":
    from ptflops import get_model_complexity_info
    
    model = multi_transformer(n_blocks=4)
