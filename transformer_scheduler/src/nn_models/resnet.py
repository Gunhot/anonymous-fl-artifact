...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, model_rate=1.0):
        super(ResNet, self).__init__()

        c1 = max(1, int(math.ceil(64  * model_rate)))
        c2 = max(1, int(math.ceil(128 * model_rate)))
        c3 = max(1, int(math.ceil(256 * model_rate)))
        c4 = max(1, int(math.ceil(512 * model_rate)))

        self.in_planes = c1
        self.conv1  = nn.Conv2d(3, c1, kernel_size=7, stride=2, padding=3, bias=False)
        ...
        self.linear = nn.Linear(c4 * block.expansion, num_classes)
...
class ResNet_cifar(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, model_rate=1.0):
        ...
        c1 = max(1, int(math.ceil(16 * model_rate)))
        c2 = max(1, int(math.ceil(32 * model_rate)))
        c3 = max(1, int(math.ceil(64 * model_rate)))
        self.in_planes = c1
        ...
        self.layer2 = self._make_layer(block, c2, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, c3, num_blocks[2], stride=2)
        self.linear = nn.Linear(c3 * block.expansion, num_classes)
...
# CIFAR-10 models
def ResNet20(num_classes=10, model_rate=1.0):
    depth = 20
    n = (depth - 2) // 6
    return ResNet_cifar(BasicBlock, [n,n,n], num_classes, model_rate)
